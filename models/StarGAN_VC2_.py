...
class ConditionalInstanceNormalization(torch.nn.Module):
    
    def __init__(self, in_ch, style_num):
        super(ConditionalInstanceNormalization, self).__init__()
        self.in_ch = in_ch
        self.style_num = style_num
        self.gamma = nn.Linear(style_num, in_ch)
        self.beta = nn.Linear(style_num, in_ch)

    def forward(self, x, code):
        
        mean = torch.mean(x, axis=2, keepdims=True)
        
        var = torch.mean((x - mean) * (x - mean), axis=2, keepdims=True)
        std = torch.sqrt(var + 1e-8)

        width = x.shape[2]
        
        gamma = self.gamma(code).view(-1, self.in_ch, 1)
        beta = self.beta(code).view(-1, self.in_ch, 1)
...
class BlockLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, style_num):
        ...
        self.cbn= ConditionalInstanceNormalization(in_ch=out_channels,
                                                  style_num=style_num)
...
class Generator(nn.Module):
    ...
    def forward(self, inputs, c, c_):
        ...
        block_layer_1 = self.blockLayer1(downconverted, c_onehot)        
        block_layer_2 = self.blockLayer2(block_layer_1, c_onehot)
        block_layer_3 = self.blockLayer3(block_layer_2, c_onehot)
        block_layer_4 = self.blockLayer4(block_layer_3, c_onehot)
        block_layer_5 = self.blockLayer5(block_layer_4, c_onehot)
        block_layer_6 = self.blockLayer6(block_layer_5, c_onehot)
        block_layer_7 = self.blockLayer7(block_layer_6, c_onehot)
        block_layer_8 = self.blockLayer8(block_layer_7, c_onehot)
        block_layer_9 = self.blockLayer9(block_layer_8, c_onehot)
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.projection = nn.Linear(self.num_classes*2, 512)
